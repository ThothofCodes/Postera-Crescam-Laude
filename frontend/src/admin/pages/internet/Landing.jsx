// PCL — Internet Distribution Landing (Branch: Signal)
...
export default function InternetLanding() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const { data } = await api.get('/admin/departments/internet/stats', { headers: { Authorization: `Bearer ${token}` } });
        setStats(data);
      } catch { setStats(null); }
      setLoading(false);
    })();
  ...
  const kpis = [
    { label: 'ACTIVE CLIENTS', value: stats?.activeClients || 0, clr: COLOR },
    { label: 'MONTHLY REVENUE', value: formatKES(stats?.revenue || 0), clr: '#39FF88' },
    { label: 'BANDWIDTH USED', value: `${stats?.bandwidthUsed || 0} GB`, clr: '#FFB020' },
    { label: 'UPTIME', value: `${stats?.uptime || 99.9}%`, clr: '#39FF88' },
    { label: 'OPEN TICKETS', value: stats?.openTickets || 0, clr: '#FF3B3B' },
    { label: 'AVG LATENCY', value: `${stats?.avgLatency || 12} ms`, clr: COLOR },
  ];
    ...
    <div style={{ padding: '1.5rem' }}>
      <div style={{ marginBottom: '1.5rem' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 4 }}>
          ...
          <span style={{ fontFamily: "'Share Tech Mono',monospace", fontSize: 10, color: COLOR, letterSpacing: '0.15em', textTransform: 'uppercase' }}>Branch: Signal</span>
        </div>
        <h2 style={{ fontSize: 28, fontWeight: 700, color: '#F4F1EA', fontFamily: "'Rajdhani',sans-serif", margin: 0 }}>Internet Distribution</h2>
        <p style={{ fontSize: 13, color: '#6A8A82', marginTop: 4 }}>Connectivity management, bandwidth allocation, and ISP client oversight</p>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '0.75rem' }}>
        {[
          { icon: "📡", title: "Manage Clients", desc: "View and manage ISP subscriptions", route: "clients" },
          { icon: "📊", title: "Bandwidth Monitor", desc: "Real-time network utilization", route: "transactions" },
          { icon: "🔧", title: "Outage Tracker", desc: "Report and track connectivity issues", route: "tickets" },
          { icon: "💰", title: "Billing", desc: "Client invoices and payments", route: "billing" },
