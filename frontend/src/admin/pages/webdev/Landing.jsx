// PCL — Web Development Landing (Branch: Forge)
...
export default function WebDevLanding() {
  ...
  useEffect(() => {
    (async () => {
      try { const { data } = await api.get('/admin/departments/webdev/stats', { headers: { Authorization: `Bearer ${token}` } }); setStats(data); } catch { setStats(null); }
      setLoading(false);
    })();
  }, []);
  ...
  const kpis = [
    { label: 'ACTIVE PROJECTS', value: stats?.activeProjects || 0, clr: COLOR },
    { label: 'MONTHLY REVENUE', value: formatKES(stats?.revenue || 0), clr: '#39FF88' },
    { label: 'COMPLETED', value: stats?.completed || 0, clr: '#2BB6A3' },
    { label: 'IN PROGRESS', value: stats?.inProgress || 0, clr: '#FFB020' },
    { label: 'CLIENTS', value: stats?.clients || 0, clr: '#60A5FA' },
    { label: 'AVG DELIVERY', value: `${stats?.avgDays || 14} days`, clr: COLOR },
  ];
    ...
    <div style={{ padding: '1.5rem' }}>
      <div style={{ marginBottom: '1.5rem' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 4 }}>
          ...
          <span style={{ fontFamily: "'Share Tech Mono',monospace", fontSize: 10, color: COLOR, letterSpacing: '0.15em', textTransform: 'uppercase' }}>Branch: Forge</span>
        </div>
        <h2 style={{ fontSize: 28, fontWeight: 700, color: '#F4F1EA', fontFamily: "'Rajdhani',sans-serif", margin: 0 }}>Web Development</h2>
        <p style={{ fontSize: 13, color: '#6A8A82', marginTop: 4 }}>Project pipeline, sprint tracking, and deployment management</p>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '0.75rem' }}>
        {[
          { icon: "🚀", title: "New Project", desc: "Create a new web development project", route: "projects" },
          { icon: "📋", title: "Project Board", desc: "Kanban view of all active projects", route: "projects" },
          { icon: "🌐", title: "Domain Manager", desc: "DNS, hosting, and SSL management", route: "crm" },
          { icon: "📈", title: "Analytics", desc: "Client site performance metrics", route: "transactions" },
